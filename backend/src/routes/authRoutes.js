...
router.post('/google-login', async (req, res) => {
  try {
    ...
    if (!token) {
      return res.status(400).json({ success: false, message: 'Google token is required', statusCode: 400 });
    }
    ...
    try {
      ...
      return res.status(400).json({ success: false, message: 'Invalid Google token', error: verifyError.message, statusCode: 400 });
    }

    const { email, name, picture } = payload;
    if (!email) {
      return res.status(400).json({ success: false, message: 'Email not provided by Google', statusCode: 400 });
    }
    ...
    if (user.status === 'blocked') {
      return res.status(403).json({ success: false, message: 'Your account has been blocked', statusCode: 403 });
    }
    ...
    res.status(500).json({ success: false, message: 'Server error during Google Authentication', error: error.message, statusCode: 500 });
  }
});
